...
@objc class AppDelegate: FlutterAppDelegate {
    ...
    override func application(
        ...
        audioRecordChannel.setMethodCallHandler({
            ...
            case "startRecord":
                self.startRecord()
            case "getAudioFloatArray":
                result(self.getAudioFloatArray())
            case "sendMfccData":
                if let args = call.arguments as? [String: Any], let mfcc = args["mfcc"] as? [Float] {
                    self.mfccData = mfcc
                    print("Received MFCC data: \(self.mfccData)")
                }
                result("MFCC data received")
            case "closeRecorder":
                result(self.closeRecorder())
            default:
    ...
    public func requestPermissionAndCreateRecorder(sampleRate: Int,
        ...
        AVAudioSession.sharedInstance().requestRecordPermission { granted in
            if granted {
                ...
                print("Please grant this app recording permission.")
                self.flutterResult!(false)
            }
        }
    }
    ...
    private func createRecorder() {
        let inputNode = audioEngine.inputNode
        let inputFormat = inputNode.outputFormat(forBus: 0)
        guard let recordingFormat = AVAudioFormat(commonFormat: .pcmFormatFloat32, sampleRate: Double(sampleRate), channels: 1, interleaved: true),
              let formatConverter = AVAudioConverter(from: inputFormat, to: recordingFormat) else { return }
        
        inputNode.installTap(onBus: 0, bufferSize: AVAudioFrameCount(bufferSize), format: inputFormat) {
            buffer, _ in
            guard let pcmBuffer = AVAudioPCMBuffer(pcmFormat: recordingFormat, frameCapacity: AVAudioFrameCount(recordingFormat.sampleRate * 2.0)) else { return }
